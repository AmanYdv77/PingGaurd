...
app = FastAPI(
    ...
    description=(
        ...
        "All requests are validated via Pydantic v2 schemas before reaching business logic."
    ),
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def health_check() -> dict[str, str]:
    ...
    return {
        ...
        "milestone": "Chapter 1 — The Request Layer",
    }
...
@app.post(
    ...
    description=(
        "Validates and registers a new monitor endpoint. "
...
async def create_monitor(
    ...
    """
    Asynchronously registers a target URL for monitoring.
...
@app.get(
    ...
    description="Retrieves configuration and status for a single monitor by its integer ID.",
)
...
@app.patch(
    ...
    description="Partially updates an existing monitor's name or check interval.",
)
@app.put(
    "/monitors/{monitor_id}",
    response_model=MonitorRead,
    ...
    description="Updates an existing monitor's configuration (name or check interval).",
)
...
@app.get(
    ...
    description="Retrieves a paginated list of registered monitors.",
)
