...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "keep_alive_enabled", sa.Boolean(), server_default=sa.text("false"), nullable=False
        ),
    ...
    op.create_index(
        op.f("ix_monitors_next_keep_alive_at"), "monitors", ["next_keep_alive_at"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["monitor_id"],
            ["monitors.id"],
            name=op.f("fk_ping_results_monitor_id_monitors"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_ping_results")),
    )
    op.create_index(
        op.f("ix_ping_results_checked_at"), "ping_results", ["checked_at"], unique=False
    )
    op.create_index(
        op.f("ix_ping_results_monitor_id"), "ping_results", ["monitor_id"], unique=False
    )
    op.create_index(
        "idx_ping_results_monitor_checked",
        "ping_results",
